...
def _to_pdf(blocks: list[Block]) -> bytes:
    ...
    heading_styles = [
        ParagraphStyle(f"h{i}", parent=body, fontName=font, fontSize=18 - i * 1.5, leading=22 - i * 2, spaceAfter=8)
        for i in range(1, 7)
    ]
    quote = ParagraphStyle("quote", parent=body, fontName=font, leftIndent=1 * cm, textColor="#555555", spaceAfter=8)
    buffer = io.BytesIO()
    story: list = []
    for block in blocks:
        if block.type == "heading":
            story.append(Paragraph(block.text.replace("&", "&amp;"), heading_styles[min(block.level or 1, 6) - 1]))
        elif block.type == "list_item":
            story.append(ListFlowable([Paragraph(block.text.replace("&", "&amp;"), body)], bulletType="bullet", leftIndent=16))
        elif block.type == "blockquote":
            story.append(Paragraph(block.text.replace("&", "&amp;"), quote))
        else:
            story.append(Paragraph(block.text.replace("&", "&amp;"), body))
        story.append(Spacer(1, 6))
    SimpleDocTemplate(buffer, pagesize=A4, rightMargin=2 * cm, leftMargin=2 * cm, topMargin=2 * cm, bottomMargin=2 * cm).build(story)
    return buffer.getvalue()
...
def export_document(blocks: list[Block], fmt: str) -> tuple[bytes, str]:
    ...
    if fmt not in MEDIA_TYPES:
        raise ValidationError(f"Unknown export format '{fmt}'. Allowed: docx, pdf.", "INVALID_FORMAT")
